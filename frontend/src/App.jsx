...
try {
  // 🔥 FIRST REQUEST (Render may be asleep)
  let response = await fetch(ANALYZE_URL, {
    method: "POST",
    body: formData,
  });

  // 🔁 RETRY ON FAILURE (for Render cold start)
  if (!response.ok) {
  ...
  setError(
    "Backend may be sleeping (Render free tier). Click Analyze again."
  );
} finally {
  setLoading(false);
}
...
return ( <div className="relative min-h-screen"> <LiveStatus /> <div className="pointer-events-none fixed inset-0 -z-10 bg-gray-950" /> <div className="pointer-events-none fixed inset-0 -z-10 bg-[radial-gradient(ellipse_120%_80%_at_50%_-20%,rgba(76,29,149,0.35),transparent_50%),radial-gradient(ellipse_80%_50%_at_100%_0%,rgba(37,99,235,0.18),transparent),radial-gradient(ellipse_60%_40%_at_0%_100%,rgba(6,182,212,0.08),transparent)]" />
  ...
  <div className="mx-auto flex min-h-screen max-w-6xl flex-col px-4 py-10 sm:px-6 lg:px-8">
    ...
    <footer className="mt-16 border-t border-white/5 py-8 text-center font-mono text-xs text-slate-600">
      ...
      <code className="rounded bg-slate-900/80 px-2 py-0.5 text-slate-400">
        {ANALYZE_URL}
      </code>
